...
def indicator_optimization(indicator_series, date, cov_estimator="shrinkage",
    ...
    weighted_stocks = sorted(set(weighted_stocks)&set(union_stks))
    ...
    not_weighted_stks = set(union_stks) - set(weighted_stocks)
    bnds1 = {s: (0, 0) for s in not_weighted_stks}
    bounds.update(bnds1)
...
def volatility_minimization(order_book_ids,date,cov_estimator="shrinkage",window=126,bounds={},industryConstraints={},styleConstraints={},benchmarkOptions=None):
    ...
    _index_components = index_components(benchmark, date=date)
    union_stks = sorted(set(_index_components).union(set(order_book_ids)))
    start_date = pd.Timestamp(date) - np.timedelta64(window * 2, "D")
...
def trackingError_minization(order_book_ids,date,cov_estimator_trackingError="shrinkage",benchmark="000300.XSHG",riskThreshold=None,cov_estimator="shrinkage",window=126,bounds={},industryConstraints={},styleConstraints={},benchmarkOptions=None):
    ...
    :param order_book_ids:股票列表
    :param date: 优化日期 如"2018-06-20"
    :param cov_estimator_trackingError 跟踪误差的协方差矩阵
            可选参数：
                -"shrinkage" 默认值; 对收益的协方差矩阵进行收缩矩阵
                -"empirical" 计算个股收益的经验协方差矩阵
    :param benchmark: string 组合跟踪的基准指数, 默认为"000300.XSHG"
    :param riskThreshold 可容忍的最大投资组合波动率
    :param cov_estimator: str 波动率约束
            可选参数：
                -"shrinkage" 默认值; 对收益的协方差矩阵进行收缩矩阵
                -"empirical" 计算个股收益的经验协方差矩阵
    ...
    lw = LedoitWolf()
    covMat = lw.fit(daily_returns).covariance_ if cov_estimator == "shrinkage" else daily_returns.cov()
    covMat_trackingError = lw.fit(
        daily_returns).covariance_ if cov_estimator_trackingError == "shrinkage" else daily_returns.cov()
    ...
    def objectiveFunction(x):
        # 投资组合波动率
        values = trackingError(x, benchmark, union_stks, date,covMat_trackingError)
        return values
